...

{
  ...
  const imagesListElements = Array.from(
    document.getElementsByClassName("filtered-image")
  );
  ...
  function updateTextPaths() {
    ...
    specialWords.forEach((specialWord) => {
      ...
      if (el) {
        ...
        const image = images.find(({ word }) => word === specialWord.wordId);
        if (isInRange) {
  ...
  function isSpecialWord(word) {
    return Boolean(
      specialWords.find(({ wordId: specialWord }) =>
